...
describe("GET /api/firmware-campaigns/[id]", () => {
  it("SHOULD proxy to the backend endpoint scoped to the campaign id", async () => {
    await GET(requestOf("/api/firmware-campaigns/campaign-1"), {
      params: Promise.resolve({ id: "campaign-1" }),
    });

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/firmware-campaigns/campaign-1");
    expect(init.method).toBe("GET");
  });
});

describe("DELETE /api/firmware-campaigns/[id]", () => {
  it("SHOULD proxy to the backend endpoint scoped to the campaign id", async () => {
    await DELETE(requestOf("/api/firmware-campaigns/campaign-1", { method: "DELETE" }), {
      params: Promise.resolve({ id: "campaign-1" }),
    });
