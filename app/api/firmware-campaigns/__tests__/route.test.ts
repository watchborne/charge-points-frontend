...
describe("GET /api/firmware-campaigns", () => {
  it("SHOULD proxy the request to the backend /api/firmware-campaigns endpoint", async () => {
    await GET(requestOf("/api/firmware-campaigns"));

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/firmware-campaigns");
    expect(init.method).toBe("GET");
  });
});

describe("POST /api/firmware-campaigns", () => {
  it("SHOULD proxy the request body to the backend /api/firmware-campaigns endpoint", async () => {
    const body = JSON.stringify({ name: "Campaign A" });

    await POST(requestOf("/api/firmware-campaigns", { method: "POST", body }));
