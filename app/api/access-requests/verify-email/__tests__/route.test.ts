...
describe("POST /api/access-requests/verify-email", () => {
  it("SHOULD proxy the request to the backend /api/access-requests/verify-email endpoint", async () => {
    await POST(requestOf("/api/access-requests/verify-email", { method: "POST", body: "{}" }));

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/access-requests/verify-email");
