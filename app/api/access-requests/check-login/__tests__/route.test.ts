...
describe("POST /api/access-requests/check-login", () => {
  it("SHOULD proxy the request to the backend /api/access-requests/check-login endpoint", async () => {
    await POST(requestOf("/api/access-requests/check-login", { method: "POST", body: "{}" }));

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/access-requests/check-login");
