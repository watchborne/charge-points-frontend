...
describe("POST /api/access-requests", () => {
  it("SHOULD proxy the request to the backend /api/access-requests endpoint", async () => {
    await POST(requestOf("/api/access-requests", { method: "POST", body: "{}" }));

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/access-requests");
