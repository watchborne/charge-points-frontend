...
describe("GET /api/charge-points/[id]", () => {
  it("SHOULD proxy to the backend endpoint scoped to the charge point id", async () => {
    await GET(requestOf("/api/charge-points/cp-1"), paramsOf("cp-1"));

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/charge-points/cp-1");
    expect(init.method).toBe("GET");
  });
});

describe("PATCH /api/charge-points/[id]", () => {
  it("SHOULD proxy the request body to the backend endpoint scoped to the charge point id", async () => {
    ...
    await PATCH(requestOf("/api/charge-points/cp-1", { method: "PATCH", body }), paramsOf("cp-1"));
...
describe("DELETE /api/charge-points/[id]", () => {
  it("SHOULD proxy to the backend endpoint scoped to the charge point id", async () => {
    await DELETE(requestOf("/api/charge-points/cp-1", { method: "DELETE" }), paramsOf("cp-1"));
