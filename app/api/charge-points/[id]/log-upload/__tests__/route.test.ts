...
describe("GET /api/charge-points/[id]/log-upload", () => {
  it("SHOULD proxy to the backend endpoint scoped to the charge point id", async () => {
    await GET(requestOf("/api/charge-points/cp-1/log-upload"), paramsOf("cp-1"));

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/charge-points/cp-1/log-upload");
    expect(init.method).toBe("GET");
  });
});

describe("POST /api/charge-points/[id]/log-upload", () => {
  it("SHOULD proxy the request body to the backend endpoint scoped to the charge point id", async () => {
    const body = JSON.stringify({
      logType: "DiagnosticsLog",
      remoteLocation: "https://example.com/upload",
    });

    await POST(
      requestOf("/api/charge-points/cp-1/log-upload", { method: "POST", body }),
      paramsOf("cp-1"),
    );
