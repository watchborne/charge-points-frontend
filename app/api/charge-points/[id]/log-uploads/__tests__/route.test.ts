...
describe("GET /api/charge-points/[id]/log-uploads", () => {
  it("SHOULD proxy to the backend endpoint scoped to the charge point id", async () => {
    await GET(requestOf("/api/charge-points/cp-1/log-uploads"), {
      params: Promise.resolve({ id: "cp-1" }),
    });

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/charge-points/cp-1/log-uploads");
