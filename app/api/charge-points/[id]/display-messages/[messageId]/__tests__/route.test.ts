...
describe("DELETE /api/charge-points/[id]/display-messages/[messageId]", () => {
  it("SHOULD proxy to the backend endpoint scoped to both the charge point id and the message id", async () => {
    await DELETE(requestOf("/api/charge-points/cp-1/display-messages/1", { method: "DELETE" }), {
      params: Promise.resolve({ id: "cp-1", messageId: "1" }),
    });

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/charge-points/cp-1/display-messages/1");
