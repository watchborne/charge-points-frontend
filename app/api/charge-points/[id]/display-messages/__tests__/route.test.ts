...
describe("POST /api/charge-points/[id]/display-messages", () => {
  it("SHOULD proxy the request body to the backend endpoint scoped to the charge point id", async () => {
    const body = JSON.stringify({ message: { id: 1, priority: "NormalCycle", message: {} } });

    await POST(requestOf("/api/charge-points/cp-1/display-messages", { method: "POST", body }), {
      params: Promise.resolve({ id: "cp-1" }),
    });

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/charge-points/cp-1/display-messages");
