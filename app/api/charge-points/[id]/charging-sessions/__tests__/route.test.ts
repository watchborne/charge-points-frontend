...
describe("GET /api/charge-points/[id]/charging-sessions", () => {
  it("SHOULD proxy to the backend endpoint scoped to the charge point id", async () => {
    await GET(requestOf("/api/charge-points/cp-1/charging-sessions"), {
      params: Promise.resolve({ id: "cp-1" }),
    });

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/charge-points/cp-1/charging-sessions");
    expect(init.method).toBe("GET");
  });

  it("SHOULD forward the limit query parameter", async () => {
    await GET(requestOf("/api/charge-points/cp-1/charging-sessions?limit=5"), {
      params: Promise.resolve({ id: "cp-1" }),
    });

    const [url] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/charge-points/cp-1/charging-sessions?limit=5");
  });
});
