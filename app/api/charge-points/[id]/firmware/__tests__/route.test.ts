...
describe("GET /api/charge-points/[id]/firmware", () => {
  it("SHOULD proxy to the backend endpoint scoped to the charge point id", async () => {
    await GET(requestOf("/api/charge-points/cp-1/firmware"), paramsOf("cp-1"));

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/charge-points/cp-1/firmware");
    expect(init.method).toBe("GET");
  });
});

describe("POST /api/charge-points/[id]/firmware", () => {
  it("SHOULD proxy the request body to the backend endpoint scoped to the charge point id", async () => {
    const body = JSON.stringify({ location: "https://example.com/fw.bin" });

    await POST(
      requestOf("/api/charge-points/cp-1/firmware", { method: "POST", body }),
      paramsOf("cp-1"),
    );
