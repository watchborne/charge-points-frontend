...
describe("POST /api/charge-points/[id]/certificates/query", () => {
  it("SHOULD proxy the request body to the backend endpoint scoped to the charge point id", async () => {
    const body = JSON.stringify({ certificateType: ["CSMSRootCertificate"] });

    await POST(
      requestOf("/api/charge-points/cp-1/certificates/query", { method: "POST", body }),
      paramsOf("cp-1"),
    );

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/charge-points/cp-1/certificates/query");
