...
describe("POST /api/charge-points/[id]/display-message-requests", () => {
  it("SHOULD proxy the request body to the backend endpoint scoped to the charge point id", async () => {
    const body = JSON.stringify({});

    await POST(
      requestOf("/api/charge-points/cp-1/display-message-requests", { method: "POST", body }),
      {
    ...
    expect(url).toBe("http://localhost:3000/api/charge-points/cp-1/display-message-requests");
