...
describe("POST /api/charge-points/[id]/availability", () => {
  it("SHOULD proxy the request body to the backend endpoint scoped to the charge point id", async () => {
    const body = JSON.stringify({ connectorId: 0, type: "Inoperative" });

    await POST(requestOf("/api/charge-points/cp-1/availability", { method: "POST", body }), {
      params: Promise.resolve({ id: "cp-1" }),
    });

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/charge-points/cp-1/availability");
