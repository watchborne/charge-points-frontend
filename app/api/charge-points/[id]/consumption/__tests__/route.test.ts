...
describe("GET /api/charge-points/[id]/consumption", () => {
  it("SHOULD proxy to the backend endpoint scoped to the charge point id", async () => {
    await GET(requestOf("/api/charge-points/cp-1/consumption?measurand=Voltage&measurand=SoC"), {
      params: Promise.resolve({ id: "cp-1" }),
    });

    const [url, init] = fetchCall();
    const parsed = new URL(url);
    expect(parsed.pathname).toBe("/api/charge-points/cp-1/consumption");
    expect(parsed.searchParams.getAll("measurand")).toEqual(["Voltage", "SoC"]);
