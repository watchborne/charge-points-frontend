...
describe("GET /api/charge-points/reliability", () => {
  it("SHOULD proxy to the backend's fleet-wide reliability endpoint", async () => {
    await GET(requestOf("/api/charge-points/reliability"));

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/charge-points/reliability");
  ...
  it("SHOULD forward from/to query parameters", async () => {
    await GET(
      requestOf(
        "/api/charge-points/reliability?from=2026-08-01T00:00:00.000Z&to=2026-08-31T00:00:00.000Z",
      ),
    );
    ...
    expect(parsed.origin + parsed.pathname).toBe(
      "http://localhost:3000/api/charge-points/reliability",
    );
    expect(parsed.searchParams.get("from")).toBe("2026-08-01T00:00:00.000Z");
    expect(parsed.searchParams.get("to")).toBe("2026-08-31T00:00:00.000Z");
  });
});
