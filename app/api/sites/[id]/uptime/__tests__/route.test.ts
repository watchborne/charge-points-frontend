...
describe("GET /api/sites/[id]/uptime", () => {
  it("SHOULD proxy to the backend endpoint scoped to the site id", async () => {
    await GET(requestOf("/api/sites/site-1/uptime"), {
      params: Promise.resolve({ id: "site-1" }),
    });

    const [url, init] = fetchCall();
    expect(url).toBe("http://localhost:3000/api/sites/site-1/uptime");
  ...
  it("SHOULD forward from/to query parameters", async () => {
    await GET(
      requestOf(
        "/api/sites/site-1/uptime?from=2026-08-01T00:00:00.000Z&to=2026-08-02T00:00:00.000Z",
      ),
      { params: Promise.resolve({ id: "site-1" }) },
    );
