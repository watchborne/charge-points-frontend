...
export const Header = () => {
  ...
  const webSocketConnectionStatus = useMemo(() => {
    ...
    switch (status) {
      ...
      case "CONNECTED":
        return <CheckCircle className={classNames(sizing, "text-st-available-500")} />;
      case "DISCONNECTED":
        return <XCircle className={classNames(sizing, "text-st-error-500")} />;
      case "ERROR":
        return <AlertCircle className={classNames(sizing, "text-st-charging-500")} />;
      default:
        return null;
    }
  }, [status]);
